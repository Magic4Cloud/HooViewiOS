...
{
    self.viewUnder=[[UIView alloc]initWithFrame:CGRectMake(((ScreenWidth / self.contentVC.childViewControllers.count) - 30 )/2, (_topViewHeight)-2,30, 2)];
    self.viewUnder.backgroundColor=[UIColor evMainColor];
...
-(void)underViewMoveTo:(int)index{
    [UIView animateWithDuration:0.7 animations:^{
        self.viewUnder.frame=CGRectMake((index *((_topViewWidth)/self.index)) + ((ScreenWidth / self.contentVC.childViewControllers.count) - 30 )/2, (_topViewHeight)-2, 30, 2);
    }];
}
