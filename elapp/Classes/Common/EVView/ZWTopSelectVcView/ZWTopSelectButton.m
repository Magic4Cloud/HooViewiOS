...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        labName.textColor=[UIColor evMainColor];
        labName.font=[UIFont systemFontOfSize:14];
        [self addSubview:labName];
        self.labName=labName;
        
        UIView *viewLine=[[UIView alloc]init];
        viewLine.backgroundColor=[UIColor evTextColorH2];
...
-(void)setState:(BOOL)state{
    if (state==YES) {
        self.labName.textColor=self.selectedColor?self.selectedColor:[UIColor evMainColor];
