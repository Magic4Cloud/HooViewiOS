...
@property (nonatomic, strong) UIButton *bigTitle_btn;
@property (nonatomic, strong) UIButton *selete_btn;
/** 标题按钮 */
...
- (void)SG_setUpSegmentedControlType:(void(^)(SGSegmentedControlStaticType *segmentedControlStaticType, NSArray **nomalImageArr, NSArray **selectedImageArr))segmentedControlTypeBlock {
    ...
    }else if (_segmentedControlStaticType == SGSegmentedControlStaticTypeBigTitle) {
        [self bigTitle_btn];
    }else {
        [self title_btn];
    }
...
- (UIButton *)bigTitle_btn {
    if (!_bigTitle_btn) {
        ...
        for (NSInteger i = 0; i < _title_Arr.count; i++) {
            // 创建静止时的标题button
            self.bigTitle_btn = [UIButton buttonWithType:(UIButtonTypeCustom)];
            
            _bigTitle_btn.titleLabel.font = [UIFont systemFontOfSize:SG_defaultTitleFont];
            _bigTitle_btn.tag = i;
            
            // 计算title_btn的x值
            button_X = i * button_W;
            _bigTitle_btn.frame = CGRectMake(button_X, button_Y, button_W, button_H);
            
            [_bigTitle_btn setTitle:_title_Arr[i] forState:(UIControlStateNormal)];
            [_bigTitle_btn setTitleColor:SG_titleColor forState:(UIControlStateNormal)];
            [_bigTitle_btn setTitleColor:SG_selectedTitleColor forState:(UIControlStateSelected)];
            // 点击事件
            [_bigTitle_btn addTarget:self action:@selector(buttonAction:) forControlEvents:(UIControlEventTouchUpInside)];
            
            // 默认选中第0个button
            if (i == 0) {
                [self buttonAction:_bigTitle_btn];
            }
            
            // 存入所有的title_btn
            [self.storageAlltitleBtn_mArr addObject:_bigTitle_btn];
            [self addSubview:_bigTitle_btn];
        }
    ...
    [UIView animateWithDuration:SG_indicatorAnimationTime animations:^{
        if (self.isFull) {
            ...
            if (_isButtonH) {
                ...
                self.indicatorView.SG_width = 16;
            }
        }
        self.indicatorView.SG_centerX = button.SG_centerX;
        if (_segmentedControlStaticType == SGSegmentedControlStaticTypeBigTitle) {
            self.selete_btn.titleLabel.font = [UIFont systemFontOfSize:SG_defaultTitleFont];
            button.titleLabel.font = [UIFont textFontB1];
            self.selete_btn = button;
        }
    }];
}
...
- (void)changeSelectButtonIndex:(NSInteger)index
{
    UIButton *selectedBtn = self.storageAlltitleBtn_mArr[index];
