...
#define ST_PULLTOREFRESH_HEADER_HEIGHT  54
...
//                _swipeHeaderBar.st_bottom  = topMarginOffset;
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(updateTableViewFloat:)]) {
                [self.delegate updateTableViewFloat:newOffsetY];
            }
        
#else
            CGFloat topMarginOffset = _headerInset - 86;
            // stick the bar
            if (newOffsetY < topMarginOffset) {
                if (_swipeHeaderView) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(swipeTableViewDidScroll:)]) {
        [_delegate swipeTableViewDidScroll:scrollView];
    }
}
