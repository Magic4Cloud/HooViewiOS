...
@interface EVEaseMob () <IChatManagerDelegate>
{
...
- (void)setUp
{
    ...
    [CCNotificationCenter addObserver:self selector:@selector(memoryWarning) name:UIApplicationDidReceiveMemoryWarningNotification object:nil];
    [CCNotificationCenter addObserver:self selector:@selector(enterForGround) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
+ (void)checkAndAutoReloginWithLoginInfo:(EVLoginInfo *)info imHasLogin:(void(^)(EVLoginInfo *loginInfo))hasLogin loginSuccess:(void(^)(EVLoginInfo *login))loginSuccess loginFail:(void(^)(EMError *error))loginFail sessionExpire:(void(^)())sessionExpire needRegistIM:(void(^)())needRegistIM
{
    EVLoginInfo *login = info;
    if ( info == nil )
    {
        login = [EVLoginInfo localObject];
    }
    ...
    {
        if ( [EVBaseToolManager imAccountHasLogin] )
        {
            if ( hasLogin )
            {
                hasLogin(login);
        ...
        {
            [EVBaseToolManager imLoginWithLoginInfo:login success:^(CCLoginIMInfo *imInfo) {
                if ( loginSuccess )
                {
                    loginSuccess(login);
                }
            } fail:loginFail sessionExpire:sessionExpire];
        }
    }
}
...
- (void)registForAppWithOptions:(NSDictionary *)launchOptions
{
    ...
    EMError *error = [[EaseMob sharedInstance] registerSDKWithAppKey:EaseMobAPPKey
                                       apnsCertName:EaseMobCerName
                                        otherConfig:@{kSDKConfigEnableConsoleLogger:@(log)}];
    if ( error )
    {
        CCLog(@"ease mob register failure");
    }
...
{
    CCLog(@"已经同意并且加入群组后的回调---");
}

// 绑定deviceToken回调
- (void)didBindDeviceWithError:(EMError *)error
{
    CCLog(@"绑定deviceToken回调----");
}
