    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *stop) {
        ...
        switch ( ivarModel.ivarType ) {
            ...
            case GPIvarPropertyDirectKeyValueType: // 可以直接使用KVC赋值的属性
                ...
                } else if ( [value isKindOfClass:[NSString class]] &&  [[GPIvar boolTypeArray] containsObject:ivarModel.typeEncode] ) {
                    value = [value isEqualToString:@"0"] ? @(NO) : @(YES);
                } else if ( [value isKindOfClass:[NSString class]] && [[GPIvar floatTypeArray] containsObject:ivarModel.typeEncode] ){
                    value = @([value floatValue]);
                } else if ( [value isKindOfClass:[NSString class]] && [[GPIvar intTypeArray] containsObject:ivarModel.typeEncode] ){
...
//else if ( [value isKindOfClass:[NSString class]] && ([ivarModel.typeEncode isEqualToString:@"c"] || [ivarModel.typeEncode isEqualToString:@"C"] || [ivarModel.typeEncode isEqualToString:@"B"] ) ) {
//    value = [value isEqualToString:@"0"] ? @(NO) : @(YES);
//}
