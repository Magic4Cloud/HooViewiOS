...
@implementation EVGroupChatViewController
...
- (void)dealloc
{
    ...
    [[EVChatMessageManager shareInstance] clearCurrentOccupantItems];
}
...
- (void)clickRightBarButtonItem
{
    ...
    EVGroupInfoViewController *infoVC = [[EVGroupInfoViewController alloc] init];
    infoVC.groupID = self.chatter;
    if ( self.memberNames.count > 0 )
    {
        infoVC.hasLoadInfo = YES;
    }
    __weak typeof(self) weakSelf = self;
    [infoVC setClearMessageSucceed:^{
        [weakSelf.dataArray removeAllObjects];
        [weakSelf.tableView reloadData];
        if ( self.delegate && [self.delegate respondsToSelector:@selector(chatViewController:didDeleteLastMessage:)] )
        {
            [self.delegate chatViewController:self didDeleteLastMessage:nil];
        }
    }];
...
- (void)setUpView
{
    ...
    EMGroup *group = [EMGroup groupWithId:self.chatter];
    ...
    self.focusViewTopContraint = [focusView autoPinEdgeToSuperviewEdge:ALEdgeTop withInset:FOCUSVIEW_HEIGHT];
    CGFloat focusViewHeight = 0;
    ...
    [tableView autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:focusView];
    ...
    if ( self.conversationItem.userModel.followed )
    {
        [self hideFocusView];
    }
    else
...
- (void)loadMessages
{
    ...
    EMConversation *conversation =  [[EVEaseMob sharedInstance].chatManager conversationForChatter:self.chatter conversationType:eConversationTypeGroupChat];
    ...
    for (EMMessage *msg in [conversation loadNumbersOfMessages:SECTION_LOAD_MESSAGE_COUNT<self.unread?self.unread:SECTION_LOAD_MESSAGE_COUNT before:[[NSDate date] timeIntervalSince1970] * 1000])
    {
        ...
        if ( msg.groupSenderName == nil )
        {
            msg.groupSenderName = [EMGroup groupWithId:self.chatter].owner;
        }
        [self.engine logoUrlWithImuser:msg.groupSenderName completion:^(NSString *logourl,NSString *name) {
            messageItem.logourl = logourl;
        }];
    }
...
- (void)handeMessage:(EMMessage *)msg
{
    ...
    __weak typeof(self) weakSelf = self;
    [self.engine logoUrlWithImuser:messageItem.message.groupSenderName completion:^(NSString *logourl,NSString *name) {
        messageItem.logourl = logourl;
        [weakSelf reloadDataWithMessage:msg];
    }];
...
- (void)markNotVoiceMessageAsRead:(EMMessage *)msg
{
    ...
    if ( [body messageBodyType] != eMessageBodyType_Voice )
    {
      ...
      [[EVChatMessageManager shareInstance] markGroupAsRead:self.chatter];
...
- (void)sendMessageWithText:(NSString *)text
{
    ...
    EMMessage *msg = [EVEaseMob sendTextMessageWithString:text toUsername:self.chatter isChatGroup:YES requireEncryption:MESSAGE_Encrypt ext:extDic];
...
{
    EMMessage *message = [EVEaseMob sendImageMessageWithImage:image toUsername:self.chatter isChatGroup:YES requireEncryption:NO ext:ext];
    [self handelSendMessage:message];
}

- (void)sendAudioMessage:(EMChatVoice *)voice
{
    EMMessageType type = eMessageTypeGroupChat;
...
- (void)chatMessageCell:(EVChatMessageCell *)cell didClickDeleteMenuWithItem:(EVMessageItem *)msgItem
{
    ...
    EMConversation *conversation = [[EVEaseMob sharedInstance].chatManager conversationForChatter:self.chatter conversationType:eConversationTypeGroupChat];
    [conversation removeMessageWithId:msg.messageId];
